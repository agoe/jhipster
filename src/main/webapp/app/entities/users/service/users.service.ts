...
export class UsersService {
  ...
  update(users: IUsers): Observable<EntityResponseType> {
    return this.http.put<IUsers>(`${this.resourceUrl}/${getUsersIdentifier(users) as string}`, users, { observe: 'response' });
  }

  partialUpdate(users: IUsers): Observable<EntityResponseType> {
    return this.http.patch<IUsers>(`${this.resourceUrl}/${getUsersIdentifier(users) as string}`, users, { observe: 'response' });
  }
